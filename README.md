...
programmed. That emergent correctness is the entire reason to model physically
...
physically exist), which is half the fun of owning the model rather than a sample.
...
*The vibration modes of a guitar-shaped orthotropic plate, solved directly from its
geometry.* Listen to a body render:
[`assets/AB_guitar_shape.wav`](assets/AB_guitar_shape.wav).

---

## What works today

The validated backend (`realsound.py`) is a small library of pure functions — each
one an isolated piece of acoustics you can test on its own:
...
- **Plate body** — an orthotropic (grain-aware) finite-difference plate solved for
  its modes, either as a square or over an **arbitrary body shape** defined by a
  boolean mask. The grain stiffness is what makes it behave like *wood* rather than
  metal.
...
- **Modal synthesis** — a bank of decaying sinusoids plus a full body impulse
...
`notebooks/guitar.ipynb` is the lab where new physics is prototyped and heard before
it graduates into `realsound.py`. The full technical write-up of how the guitar was
built lives in [`docs/RealSound_Guitar_Report.md`](docs/RealSound_Guitar_Report.md);
the project charter and roadmap are in
...
Developed on **Python 3.10**. The core runtime dependencies are just `numpy`,
`scipy`, and `matplotlib` (see [`requirements.txt`](requirements.txt)); the notebook
and the demo app pull in a couple of extras noted below.
...
The notebook uses `IPython.display.Audio` to play results inline. Its outputs are
stripped in version control — run the cells to regenerate the plots and audio.
...
It renders an A-major scale on both an integer-delay and a fractional-delay string,
measures the *actual* pitch of the rendered audio, and prints the error in cents —
demonstrating that fractional delay is what pulls the string into tune.
...
├── requirements.txt
├── .gitignore
├── realsound.py          # validated backend: string, plate, cavity, modal, sequencer, analysis
├── ks_demo.py            # standalone Karplus–Strong demo (Milestone 0)
...
`realsound.py` stays at the root for now (a clean `from realsound import ...`); it
may later become a proper `realsound/` package. Generated `.wav`/`.png` renders are
git-ignored except the curated few in `assets/`.
